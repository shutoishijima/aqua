                    ...
                    <div class="content-box flex m-b10">
                        <div class="c-box-l">
                            @php
                                ...
                                // 有料コンテンツかつ有料会員
                                } elseif ($career->content_lock == '1' && $users[0]->user_status == '1'){
                                    $video_flag = true;
                                }

                                // 最新の視聴コンテンツより一つ以上先は見れない
                                if ($latest_content[0]->content_id + 1 < $career->content_id) {
                                    $video_flag = false;
                                }

                                if ($latest_content[0]->content_id < $career->content_id) {
                        ...
                        <div class="c-box-r">
                            ...
                            <div class="content-view">
                                @foreach ($viewed_content as $view_content)
                                    @if ($career->content_id == $view_content->content_id && $view_content->content_status == 2)
                                        <p class="red">受講済み​<i class="fas fa-check red"></i></p>
                                    @endif
                                @endforeach

                                @if ($latest_content[0]->content_id + 1 == $career->content_id && $latest_content[0]->content_status == 2)
                                    @if ($latest_content[0]->content_id < $career->content_id && $users[0]->user_status == '0' && $career->content_lock == 1)
                                    ...
                                    @else
                                        ...
                                        @endif
                                    @endif
                                @elseif ($latest_content[0]->content_id < $career->content_id && $users[0]->user_status == '0' && $career->content_lock == 1)
                                    <p class="red">有料会員しか視聴できません</p>
                                @elseif ($latest_content[0]->content_id < $career->content_id && $users[0]->user_status != '1')
